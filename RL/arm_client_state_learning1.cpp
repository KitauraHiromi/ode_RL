...
void simLoop(int pause) {
  ...
  if(count % SIGNAL_SPAN == 1){
    ...
    for (int i = 0; i <NUM; i++ ){
    dsDrawCapsule(dBodyGetPosition(robot->link[i].body),
    ...
    /*
    for(int i=0; i<DOF; i++){
      dsDrawCylinder(dBodyGetPosition(robot->joint_cyli[i].body),
    ...
    */
    ...
    // Dawing tactile sensors.
    dsSetColor(1.0, 0.0, 0.0);
    for(int i=0; i<TAC_NUM; i++)
      dsDrawBox(dBodyGetPosition(robot->tac_sensor[i].body),
