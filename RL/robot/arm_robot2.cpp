...
Arm_Robot2::Arm_Robot2(dWorldID world, dSpaceID space){
  // robot configuration
  dof = NUM - 1;
  angle_pitch = 5;
  double _ROM[DOF * 2] = {0, 140, -180, 180};
  memcpy(ROM, _ROM, MEM_DOF*2);
  
  // Initialize variables
  double _ANGLE[DOF] = { 0.0, 0.0};
  double _l[NUM]  = { 0.10, 0.90, 1.0 };
  double _r[NUM]  = { 0.1, 0.04, 0.04 };
  ...
  double _x[NUM] = {0.00};
  double _y[NUM] = {0.00};
  double _z[NUM] = { l[0]/2, l[0]+l[1]/2, l[0]+l[1]+l[2]/2 };
  memcpy(x, _x, NUM);
  memcpy(y, _y, MEM_NUM);
  memcpy(z, _z, MEM_NUM);

  // mass
  double _m[NUM] = {10.00, 2.00, 2.00 };
  memcpy(m, _m, MEM_NUM);

  // tac sensor position
  double tac_x[TAC_NUM] = {0.0};
  double tac_y[TAC_NUM] = {0.0};
  double tac_z[TAC_NUM] = { l[0]+l[1]+l[2]+0.05 };

  // 回転中心
  double _anchor_x[DOF] = { 0.00 };
  double _anchor_y[DOF] = { 0.00 };
  double _anchor_z[DOF] = { 0.10, 1.00 };
  memcpy(anchor_x, _anchor_x, MEM_DOF);
  memcpy(anchor_y, _anchor_y, MEM_DOF);
  memcpy(anchor_z, _anchor_z, MEM_DOF);

  // 回転軸
  double _axis_x[DOF]   = { 0.00, 0.00 };
  double _axis_y[DOF]   = { 1.00, 2.00 };
  double _axis_z[DOF]   = { 0.00, 0.00 };
  ...
  // tactile sensor is fixed to the end link.
  for(int j=0; j<TAC_NUM; j++){
    tac_fix[j] = dJointCreateFixed(world, 0);
    dJointAttach(tac_fix[j], link[2].body, tac_sensor[j].body);
    dJointSetFixed(tac_fix[j]);
  }
}

Arm_Robot2::~Arm_Robot2(){
}

void Arm_Robot2::control() {
  /***  PID  ****/
  static int step = 0;
...
bool Arm_Robot2::Is_Tactile(dGeomID obj){
  for(int i=0; i<TAC_NUM; i++)
    if(tac_sensor[i].geom == obj) return true;
  else return false;
}

int Arm_Robot2::Which_Tactile(dGeomID obj){
  for(int i=0; i<TAC_NUM; i++)
    if(tac_sensor[i].geom == obj) return i;
  else return -1;
}

bool Arm_Robot2::BodyTactileCollision(dGeomID obj1, dGeomID obj2){
  for(int i=0; i<TAC_NUM; i++)
    for(int j=0; j<NUM; j++)
...
void Arm_Robot2::command(int cmd){
}
